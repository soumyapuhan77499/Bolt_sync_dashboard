            ...
            <tbody>
                @forelse($connections as $row)
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route('admin.database-connections.test', $row->id) }}">
                                @csrf
                                <button type="submit" class="btn btn-warning">Test</button>
                            </form>

                            <form method="POST" action="{{ route('admin.database-connections.activate', $row->id) }}">
                                @csrf
                                <button type="submit" class="btn btn-primary">Activate</button>
                            </form>

                            <form method="POST" action="{{ route('admin.database-connections.deactivate', $row->id) }}">
                                @csrf
                ...
                @empty
                    <tr>
                        <td colspan="13">No database connections found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
